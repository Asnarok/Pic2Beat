...
public class Chord implements JMC, Serializable{
	...
	public int length; //en nombre de temps de 1 � 4
	...
	public Chord(String name, ChordType type) {
		...
		switch (type) {
			case MAJ:
				notes[1] = Note.getNote(this.interval(notes[0], 4));// NotesToInt.get(modulo(NotesToInt.indexOf(Notes[0])+4, 12));
				notes[2] = Note.getNote(this.interval(notes[1], 3));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[1])+3, 12));
				break;
			case MIN:
				notes[1] = Note.getNote(this.interval(notes[0], 3));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[0])+3, 12));
				notes[2] = Note.getNote(this.interval(notes[1], 4));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[1])+4, 12));
				break;
			case MAJ7:
				notes[1] = Note.getNote(this.interval(notes[0], 4));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[0])+4, 12));
				notes[2] = Note.getNote(this.interval(notes[1], 3));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[1])+3, 12));
				notes[3] = Note.getNote(this.interval(notes[2], 4));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[2])+4, 12));
				break;
			case MIN7:
				notes[1] = Note.getNote(this.interval(notes[0], 3));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[0])+3, 12));
				notes[2] = Note.getNote(this.interval(notes[1], 4));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[1])+4, 12));
				notes[3] = Note.getNote(this.interval(notes[2], 3));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[2])+3, 12));
				break;
			case DOM7:
				notes[1] = Note.getNote(this.interval(notes[0], 4));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[0])+4, 12));
				notes[2] = Note.getNote(this.interval(notes[1], 3));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[1])+3, 12));
				notes[3] = Note.getNote(this.interval(notes[2], 3));//NotesToInt.get(modulo(NotesToInt.indexOf(Notes[2])+4, 12));
				break;
			case DIM7:
				notes[1] = Note.getNote(this.interval(notes[0], 3));
				notes[2] = Note.getNote(this.interval(notes[1], 3));
				notes[3] = Note.getNote(this.interval(notes[2], 4));
				break;
			case POWER_CHORD:
				notes[1] = Note.getNote(this.interval(notes[0], 7));
		}
	}

	public Chord(int note, ChordType type) {
		this(Note.getNote(note), type);
	}
