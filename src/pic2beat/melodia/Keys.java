...
public class Keys implements Scales {
	...
	public static final ChordType[] MAJOR_SCALE_CHORDS = { ChordType.MAJ, ChordType.MIN, ChordType.MIN, ChordType.MAJ,
			ChordType.MAJ, ChordType.MIN, ChordType.DIM };
	...
	public static List<Scale> getCompatibleScales(Chord c, Scale s) throws KeyException {
		...
		Chord[] neighborChords = getNeighborChords(c, s);
	...
	public static Chord[] getNeighborChords(Chord c, Scale s) throws KeyException {
		Chord[] scaleChords = scaleChords(s.getNote(), s.getMode());
		
		Chord simplified = c.simplifyToTriad();

		int degree = -1;
		for (int i = 0; i < scaleChords.length; i++) {
			if (scaleChords[i].equals(simplified)) {
				degree = i;
				break;
			}
		}
	...
	public static Chord[] scaleChords(int note, int mode) {
		...
		for (int i = 1; i < 7; i++) {
			scaleChords[i] = new Chord(NATURAL_MAJOR_INTERVALS[modulus(i + mode, 7)] + keyNote,
					MAJOR_SCALE_CHORDS[modulus(i + mode, 7)]);
		}
		return scaleChords;
	}
	...
	public static Chord scaleChord(int note, int mode, int degree) {
		int keyNote = note - NATURAL_MAJOR_INTERVALS[mode];
		return new Chord(NATURAL_MAJOR_INTERVALS[modulus(degree + mode, 7)] + keyNote,
				MAJOR_SCALE_CHORDS[modulus(degree + mode, 7)]);
	}
